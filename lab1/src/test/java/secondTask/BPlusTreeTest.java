...
public class BPlusTreeTest {
    ...
    public void insertRemoveAndSearch() {
        ...
        first.search(7);
        assertArrayEquals(new Integer[]{2, 3, 5, 6, 7}, first.result.toArray(new Integer[0]));
        first.search(11);
        assertArrayEquals(new int[]{2, 3, 5, 6, 7, 10, 11}, first.result.stream().mapToInt(i -> i).toArray());
        first.search(10);
        assertArrayEquals(new int[]{2, 3, 5, 6, 7, 10}, first.result.stream().mapToInt(i -> i).toArray());
    }
    ...
    void searchIfLeaf() {
        ...
        assertArrayEquals(new int[]{1, 5, 6, 7, 11, 12, 13, 19, 24, 46, 76}, first.result.stream().mapToInt(i -> i).toArray());
    }
    ...
    void checkFillPredWhileRemoving() {
        insert(arr2);
        System.out.println(first.root.children[0].traverse().toString());
        first.remove(9);
        first.remove(8);
        assertEquals("[1, 2, 3, 4, 5]", first.root.children[0].traverse().toString());
    }
